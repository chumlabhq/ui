...
const AccordionShimmer = forwardRef<HTMLDivElement, AccordionShimmerProps>(
  (
    ...
    return (
      <Comp
        ref={ref}
        className={`w-full ${variantClasses.root} ${className ?? ""}`.trim()}
        style={style}
        role="status"
      ...
      >
        {items.map((index) => (
          ...
          >
            <div className={`flex items-center justify-between ${sizeClasses.trigger}`}>
              <div className="flex-1 space-y-2">
                <div
                  className={`h-4 rounded ${shimmerClass} bg-gray-200 dark:bg-gray-700`}
                  style={{ width: `${TITLE_WIDTHS[index % TITLE_WIDTHS.length]}%` }}
                />
                {showContent && (
                  <div
                    className={`h-3 rounded ${shimmerClass} bg-gray-100 dark:bg-gray-600`}
                    style={{ width: `${SUBTITLE_WIDTHS[index % SUBTITLE_WIDTHS.length]}%` }}
                  />
                )}
              </div>
              <div
                className={`${sizeClasses.icon} rounded ${shimmerClass} bg-gray-200 dark:bg-gray-700 shrink-0 ml-4`}
              />
            </div>
            {showContent && (
