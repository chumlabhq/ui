    ...
    return (
      ...
      >
        {items.map((index) => (
          ...
          >
            <div className={cn("flex items-center justify-between", sizeClasses.trigger)}>
              <div className="flex-1 space-y-2">
                <div
                  className={cn("h-4 rounded bg-gray-200 dark:bg-gray-700", shimmerClass)}
                  style={{ width: `${TITLE_WIDTHS[index % TITLE_WIDTHS.length]}%` }}
                />
                {showContent && (
                  <div
                    className={cn("h-3 rounded bg-gray-100 dark:bg-gray-600", shimmerClass)}
                    style={{ width: `${SUBTITLE_WIDTHS[index % SUBTITLE_WIDTHS.length]}%` }}
                  />
                )}
              </div>
              <div
                className={cn(sizeClasses.icon, "rounded bg-gray-200 dark:bg-gray-700 shrink-0 ml-4", shimmerClass)}
              />
            </div>
            {showContent && (
